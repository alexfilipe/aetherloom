...
The policy governing these decisions, the default cutoff catalog, and the entry format are all in [`README.md`](README.md) beside this file. Entries recorded before 2026-08-12 cite this log at its former path, `architecture/cutoff-decisions.md`; those citations are left as written rather than rewritten, per the append-only rule.
...
## CUT-023 — Evaluation-loop framework finish line
...
- CUT-023: Published as draft PR #16. The framework was introduced by `ab6965e74786ce66a30d56e898bae0ebc1b7c5cd`; this appended metadata is the only post-publication correction.
...
- Date: 2026-08-12
- PR/layer and exact relevant SHA(s): evaluation-loop documentation in PR [#20](https://github.com/alexfilipe/aetherloom/pull/20) on branch `claude/orchestration-framework-review-07538f`. The base moved twice during review. Each integration voided the previous exact-head evidence and was revalidated, so the chain is recorded in full and **only the last row is authoritative**:
  - base `1f1b9e1202252f8c7b9c367b5c69e5aa383c0fcf` (at definition), head `84bd82a64220ca4b7466cefc4f798120cad9008c`, macOS CI [run 31583666633](https://github.com/alexfilipe/aetherloom/actions/runs/31583666633) — superseded;
  - base `c435d1c77a8052ebd0b1641a15bdb96e8eb4c492` after integrating merged PR #19, integrated by `d6ec44d1231cf28ef7aac74f6dc193193737e09f`, macOS CI [run 31583992360](https://github.com/alexfilipe/aetherloom/actions/runs/31583992360) — superseded;
  - base `d5dd1166b255a7e855eb1ce566b12b324013d63a` after integrating merged PR #24, integrated by `d35552668802669a7dda1722bd5370605a875440`, macOS CI [run 31594907255](https://github.com/alexfilipe/aetherloom/actions/runs/31594907255) — authoritative.

  Any further base integration voids the last row under E11 and requires one more exact-head run. The commit carrying this correction likewise needs one before the entry is canonical.
- Decision/cutoff: Refine the framework from a full documentation review, then, on user direction during review, reorganize the cutoff documents. The scope grew twice beyond its original definition and this entry records the final, wider scope rather than the first one. It covers: aligning cutoff E14's reopen triggers with the framework's "newly evidenced P0/P1" rule; naming where Define artifacts are recorded; defaulting severity disagreements to the higher class until resolved at a gate; adding a `reopened` status to the entry format; linking the framework and the tracks it governs from the root README; making base-integration revalidation explicit in the validation ladder and E11; relocating the decision log into `architecture/orchestration/cutoffs/`; folding `TEMPLATE.md` into `cutoffs/README.md` and deleting it; moving the procedure sections `cutoffs/README.md` restated into the framework README as their single normative home, including a new human-evaluation section there; and correcting the duplicated `CUT-009` label. No default, threshold, budget, or cutoff condition changed value: the rules were made consistent and given one home each. Documentation-only, so evaluation remains writer self-evaluation plus structural checks — reference and link integrity, E-ID definition/citation coverage, byte-identity of merged entries, and one exact-head CI run per published head — with no subagents and no independent evaluation.
...
- What was completed: The E14 alignment, Define-artifact locations, severity-disagreement default, `reopened` status, root-README and architecture-index cross-references, CUT-numbering guidance, and resolution of the duplicated `CUT-009` identifier by renumbering the later evaluation-loop framework entry to `CUT-023`. One review correction batch then resolved three documentation-consistency findings: the stale reopen rule in “Precedent and intent” that still excluded the newly covered P1 case (P1), the root README sending general scope decisions to the cutoff log against the new Define split (P2), and this entry's stale base SHA (P2). The same batch added an explicit base-integration revalidation rule to the validation ladder and E11, because this PR's own base change proved the existing evidence-reuse wording left it implicit and E11 was readable as forbidding the required fresh run. A follow-up correction then consolidated the cutoff documents into one instructions file and one log: the log moved from `architecture/cutoff-decisions.md` to `architecture/orchestration/cutoffs/DECISIONS.md`; `TEMPLATE.md` was folded into `cutoffs/README.md`, whose duplicate field list had already drifted from it; and the procedure sections `cutoffs/README.md` restated — subagent bounds, the validation ladder, the default evaluation sequence, and human-smoke routing — were removed in favour of the single normative statement in the framework README, with the content unique to them ported there first.
- What was explicitly deferred: The `AGENTS.md`/`CLAUDE.md` mirror-drift guard, routed per user direction to PR [#19](https://github.com/alexfilipe/aetherloom/pull/19), which verifies parity with a pre-commit hook rather than a symlink so the check can be deleted when the two documents must diverge; runtime enforcement tooling for this framework; migrating the pre-existing work-order and agent documents to the consolidated structure; and any change to the loop's defaults, thresholds, or evaluation budgets, which this entry deliberately leaves at their recorded values.
- Residual risk/severity: P3. No behavior changes, but the reorganization moved normative text between documents, so the risk is that a rule was dropped or silently duplicated in the move rather than that a rule is wrong. Checked by confirming all twenty E-IDs remain defined and cited, every relocated clause appears exactly once, no reference or link dangles, and the merged entries this change did not intend to touch are byte-identical to `main`.
