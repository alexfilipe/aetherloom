...
| Providers/fakes | Contract suite run against `FakeStorageProvider` (and later every real provider, opt-in): failure-never-emptiness, `.complete` proof, placeholder inclusion, precondition-enforcing stores, quarantine exclusion; deterministic mutation gates distinguish pre-start expiry, post-start late success/failure, and reconciliation release |
