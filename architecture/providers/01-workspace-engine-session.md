...
An executable preparation is exactly a plan whose `gate == .clear`, or whose `gate == .hold && gate.permitsApproval`. Before accepting execution, the bridge validates fingerprint equality; `confirmedAt <= now < expiresAt`; `expiresAt > confirmedAt`; and exact acknowledgement counts against `approvalTrashCount` and `approvalConflictCount`. Every executable preparation with a nonzero required count needs the corresponding acknowledgement; clear does not imply zero counts. The bridge then deterministically derives the core call: an approvable held plan receives `PlanApproval(planFingerprint: confirmation.planFingerprint, approvedAt: confirmation.confirmedAt, expiresAt: confirmation.expiresAt, acknowledgedTrashCount: confirmation.acknowledgedTrashCount, acknowledgedConflictCount: confirmation.acknowledgedConflictCount)`; a clear plan passes `nil` **only inside the bridge** after the same non-optional confirmation validation.

A refusal and a non-approvable hold are both unexecutable, but they remain distinct outcomes. A non-approvable hold—currently any hold containing `massDeletion`—keeps its preview and evidence visible but has no `WorkspaceExecutionConfirmation` construction, enabled **Sync Now**, approval derivation, or core executor call. It stays stopped until the user changes the underlying files or sync configuration and explicitly prepares a fresh plan whose gate is executable. The bridge rejects a non-approvable hold before any approval derivation. The `EngineSession` protocol consumed by `AppModel`, including `DemoEngineSession` and `WorkspaceEngineSession`, exposes no nullable `PlanApproval?`, Boolean approval, path enrollment, or other production escape hatch.
...
3. The app presents that preparation, including an empty plan, refusal, approvable hold, non-approvable hold, exclusion, or recovery notice.
4. If the preparation is executable, the user explicitly confirms that exact preparation after acknowledging every nonzero required trash/conflict count. Refusals and non-approvable holds remain visible and unconfirmable.
5. The session validates the non-optional confirmation bound to the preparation fingerprint, time window, and exact required counts, then calls the core executor. It derives `PlanApproval` only for an approvable hold; a validated clear plan passes internal `nil`.

The first run of a newly created set MUST use this path, even when its gate is clear. No production run, first or later, may mutate a provider without an explicit confirmation of the displayed preparation. A clear plan with no acknowledgement requirements may enable confirmation immediately; a clear plan with nonzero required counts may not. A refusal or non-approvable hold cannot be confirmed. A stale, missing, expired, mismatched, or count-inexact confirmation fails without mutation. Core's ability to execute a clear plan without a `PlanApproval` is not automatic authority for the production bridge; the bridge MUST enforce the stronger manual-product boundary.
