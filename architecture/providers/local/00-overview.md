...
- The content stage keys late fetch temporary URLs and destination-store source pins by the full stable mutation identity. Handles for one canonical stage root share a process-wide storage actor, so reconstruction cannot erase a live temporary or forget a pin. Durable recovery releases artifacts only after journal reconciliation. The first owner removes only UUID-named `.tmp` files in its dedicated root left by a previous process, while preserving unrelated files and verified `.stage` cache entries.
...
3. **Extended attributes, Finder tags, FinderInfo, and resource forks:** resolved for the MVP by typed visible exclusion except for the exact-name, live Apple sync-intent compatibility exception for `com.apple.provenance`; see [01-package-and-metadata-safety.md](01-package-and-metadata-safety.md). The exception ignores opaque platform-local state and makes no payload or preservation claim.
