...
Mutation-adjacent provider checks remain mandatory defense in depth after the preflight. Immediately before each physical commit, the provider MUST re-check the applicable classification and ordinary version/absence preconditions. The exact-name predicate is re-evaluated at scan, all-location preflight, mutation-adjacent classification, and recovery; a result that changes to preservable, unavailable, erroneous, or otherwise ambiguous stops or refuses through the existing fail-closed path before mutation or convergence. Drift arising after the all-location barrier stops the run; it never authorizes an overwrite or trash. Root container metadata remains outside synchronized content.
