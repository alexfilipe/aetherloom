...
| LW-15 | L2 | Package root, a selected directory inside a package, and encountered package subtree; file and directory metadata/resource fork; `lstat`-derived regular/directory/symlink/FIFO/socket/character-device/block-device/other/indeterminate kind treatment; exact `0644`/`0755` plus effective uid/gid baseline; ordinary umask/executable/private/foreign-owned exclusions; and non-baseline permission, special-bit, ACL, or ownership cases produce the accepted typed policy and exact durable path/reason before observation or content I/O. Realistic-volume qualification proves bounded one-root-per-exclusion evidence and inspectable display aggregation. |
