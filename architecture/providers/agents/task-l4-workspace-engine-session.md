...
- Replace the AppModel-facing nullable approval API with the exact non-optional `WorkspaceExecutionConfirmation` seam; add separate `LocalFolderDraft`/opaque `FolderAccessGrant` enrollment and reauthorization methods. Migrate `EngineSession`, `DemoEngineSession`, `WorkspaceEngineSession`, the minimum AppModel call sites, preview/test sessions, and compile/interaction tests together so the app build remains green.
- Implement `WorkspaceEngineSession` bootstrap, state reads/events, metadata-only location/sync-set operations, pause, conflict intent, prepare, confirmation validation/internal `PlanApproval?` derivation, and sync-set deletion.
...
Focused coverage MUST include non-optional confirmation for Demo and Workspace clear/held plans; deterministic held `PlanApproval` and clear internal `nil` derivation; fingerprint/time/expiry/count mismatch; compile/source proof that AppModel sees no nullable/Boolean escape hatch or path enrollment; all seven parameterized LW-08 causes; scope start/stop balance across success, refusal, cancellation, timeout, and indeterminate recovery; identity persistence before construction; overlap failure before scan; conflict-intent no-mutation; no schedule replay; metadata-only deletion; durable pause; and converged empty relaunch. Then run all bridge/core tests, the complete Swift suite, the app build, source/layering audits, and exact-head macOS/user validation.
