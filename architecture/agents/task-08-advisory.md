...
## Deliverables
### `src/AetherloomCore/Sources/AetherloomCore/Advisory/` (zero ML imports)
...
### `AetherloomIntelligence/` (new SPM target in `src/AetherloomCore/Package.swift`, depends on core only)
...
## Prohibitions
Greps that must stay clean: `import FoundationModels|CoreML|MLX` under `src/AetherloomCore/Sources/AetherloomCore` → nothing; `URLSession` under both targets → nothing. No UI. Only `src/AetherloomCore/` (package dir, both targets).
